...
Kullanim: chfNfiBisect.py girdi.ber cikti.ber 0,1,3
...
def rebuild(buf, s, e, keep, hits):
    ...
    while i < e:
        ...
        if con and is_nfi(buf, cs, ce):
            inner = bytearray()
            for _, kn, _, ki, kcs, kce in kids(buf, cs, ce):
                if kn in keep: inner += buf[ki:kce]
            hits.append(n)
            out += tb + enclen(len(inner)) + bytes(inner)
        elif con:
            inner = rebuild(buf, cs, ce, keep, hits)
...
out = rebuild(buf, 0, len(buf), keep, hits)
